...
Deno demo app for Deepgram Live Text-to-Speech.

## Architecture

- **Backend:** Deno (TypeScript) on port 8081
...
| File | Purpose |
|------|---------|
| `server.ts` | Main backend — API endpoints and WebSocket proxy |
...
- **Backend:** `deno.json` — Deno auto-installs deps. Frontend still uses pnpm. Commits deno.lock.
- **Frontend:** `frontend/package.json` — Vite dev server
- **Submodules:** `frontend/` (live-text-to-speech-html), `contracts/` (starter-contracts)

Install: `# Deno auto-resolves deps on first run`
...
4. Handle it in the backend `server.ts` — read the param and pass it to the Deepgram API
...
```
feat(deno-live-text-to-speech): add diarization support
fix(deno-live-text-to-speech): resolve WebSocket close handling
refactor(deno-live-text-to-speech): simplify session endpoint
